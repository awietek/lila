...
inline Matrix<coeff_t> Unitary(size_type n,
  ...
  for (int n_offdiag = 1; n_offdiag < n; ++n_offdiag)
    for (int j = 0; j < n - n_offdiag; ++j) {
      // Construct hermitian elementary matrix
      coeff_t entry =
          coeff_t(params(n + param_offset), params(n + param_offset + 1));
...
template <class coeff_t> inline bool IsUnitary(const Matrix<coeff_t> &mat) {
