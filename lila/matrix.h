...
template <class coeff_t> coeff_t Trace(const Matrix<coeff_t> &mat) {
  // TODO: optimize ??
  coeff_t res = 0;
...
template <class coeff_t> Matrix<coeff_t> Transpose(const Matrix<coeff_t> &mat) {
...
template <class coeff_t> Vector<coeff_t> Diag(const Matrix<coeff_t> &mat) {
...
template <class coeff_t> Matrix<coeff_t> ParseMatrix(const std::string &str) {
  using size_type = lila::size_type;
